...
test('protected endpoint requires auth; expired token is rejected', async () => {
  ...
  const expired = jwt.sign({ id: 's1', role: 'student' }, process.env.JWT_SECRET, { algorithm: 'HS256', expiresIn: -10 });
  const c = await client().prime();
...
test('student flow: dashboard data loads for own batch only', async () => {
  ...
  assert.strictEqual(me.json.batchId, 'b1');

  const mats = await c.get('/api/materials/student');
  assert.strictEqual(mats.status, 200);
  assert.ok(mats.json.every(m => m.batchId === 'b1' && m.published === true), 'only own-batch published materials');
  assert.ok(!mats.json.some(m => m.id === 'm2'), 'draft material hidden');

  const asg = await c.get('/api/assignments/student');
  assert.ok(asg.json.every(a => a.batchId === 'b1' && a.published), 'only own-batch published assignments');

  const sets = await c.get('/api/flashcards/sets/student');
  assert.ok(sets.json.every(s => s.published && s.batchId === 'b1'));
  assert.ok(!sets.json.some(s => s.id === 'fs2'), 'draft flashcard set hidden');
});
...
test('student lectures: only published for own batch; drafts and other batches hidden', async () => {
  ...
  assert.ok(allCards.some(l => l.id === 'lec_pub'), 'published own-batch lecture visible');
  assert.ok(!allCards.some(l => l.id === 'lec_draft'), 'draft lecture NOT listed');
  assert.ok(!allCards.some(l => l.id === 'lec_b2'), 'other batch lecture NOT listed');

  const pub = await c.get('/api/lectures/lec_pub/student');
  assert.strictEqual(pub.status, 200);
  assert.ok(Array.isArray(pub.json.slides) && pub.json.slides.length === 2, 'published slides render');

  assert.strictEqual((await c.get('/api/lectures/lec_draft/student')).status, 404, 'draft body hidden from students');
  assert.strictEqual((await c.get('/api/lectures/lec_b2/student')).status, 404, 'other-batch lecture body blocked');
...
test('SECURITY 2 — student cross-batch / unpublished assignment submission blocked', async () => {
  ...
  assert.strictEqual((await cross.c.post('/api/assignments/as1/submissions', { content: 'x' })).status, 403);

  const own = await loginStudent('ada@test.local');
  assert.strictEqual((await own.c.post('/api/assignments/as2/submissions', { content: 'x' })).status, 403, 'draft assignment rejects');
  assert.strictEqual((await own.c.post('/api/assignments/as1/submissions', { content: 'x' })).status, 201);
});
...
test('SECURITY 3 — flashcard grading is server-side; forged isCorrect ignored', async () => {
  ...
  void r;
  const resp = await c.post('/api/flashcards/sets/fs1/respond', {
    responses: [
      { flashcard: 'c1', answer: 'A loop', isCorrect: true },  // wrong → correct is "A container"
      { flashcard: 'c2', answer: '4', isCorrect: false },      // right
    ],
  });
  assert.strictEqual(resp.status, 201);
  // Verify via lecturer results that grading was recomputed.
  const lec = await loginLecturer('lex@test.local');
  const results = await lec.c.get('/api/flashcards/sets/fs1/results');
  const byCard = Object.fromEntries((results.json || []).map(x => [x.flashcardId, x.isCorrect]));
  assert.strictEqual(byCard.c1, false, 'forged isCorrect:true must be stored as false');
  assert.strictEqual(byCard.c2, true);
});
...
test('lecturer flow: sees only own-batch lectures; can edit + publish own', async () => {
  ...
  assert.ok(list.json.every(l => l.batchId === 'b1'), 'only own batch lectures listed');
  assert.ok(!list.json.some(l => l.id === 'lec_b2'), 'other batch lecture absent');

  const full = await c.get('/api/lectures/lec_draft');
  assert.strictEqual(full.status, 200, 'lecturer can open own draft');

  const patched = await c.patch('/api/lectures/lec_draft', { lectureTitle: 'Renamed Draft' });
  assert.strictEqual(patched.status, 200);
  assert.strictEqual(patched.json.lectureTitle, 'Renamed Draft');

  const pub = await c.post('/api/lectures/lec_draft/publish', {});
  assert.strictEqual(pub.status, 200);
...
test('SECURITY 4 — lecturer cross-batch/cross-lecturer IDOR blocked across resources', async () => {
  ...
  const c = lena.c;
  // lectures
  assert.strictEqual((await c.get('/api/lectures/lec_pub')).status, 403);
  assert.strictEqual((await c.patch('/api/lectures/lec_pub', { lectureTitle: 'hax' })).status, 403);
  assert.strictEqual((await c.post('/api/lectures/lec_pub/publish', {})).status, 403);
  assert.strictEqual((await c.post('/api/lectures/generate', { batchId: 'b1', week: 1, day: 'Wednesday' })).status, 403);
  // materials / assignments / flashcards / attendance / curriculum
  assert.strictEqual((await c.get('/api/materials/m1')).status, 403);
  assert.strictEqual((await c.patch('/api/materials/m1', { title: 'hax' })).status, 403);
  assert.strictEqual((await c.get('/api/assignments/as1')).status, 403);
  assert.strictEqual((await c.get('/api/assignments/as1/submissions')).status, 403);
  assert.strictEqual((await c.patch('/api/flashcards/sets/fs1', { title: 'hax' })).status, 403);
  assert.strictEqual((await c.get('/api/flashcards/sets/fs1/results')).status, 403);
  assert.strictEqual((await c.patch('/api/attendance/att1/open', {})).status, 403);
  assert.strictEqual((await c.get('/api/attendance/att1')).status, 403);
});
...
test('admin flow: login + list applicants/students/batches; unrestricted lecture access', async () => {
  ...
  assert.strictEqual((await c.get('/api/lectures/lec_b2')).status, 200);
  assert.strictEqual((await c.get('/api/attendance/att1')).status, 200);
});
...
test('no cross-batch data leak in student payloads', async () => {
  const { c } = await loginStudent('ada@test.local');
  const sets = await c.get('/api/flashcards/sets/student');
  assert.ok(!JSON.stringify(sets.json).includes('b2'), 'no batch-b2 ids in student flashcard payload');
  const mats = await c.get('/api/materials/student');
  assert.ok(mats.json.every(m => m.batchId === 'b1'));
});
