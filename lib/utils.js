...
const JSONB_FIELDS = new Set([
  ...
  'class_days',
  // Lecture content blobs — their internal keys (main_sections, on_slide_text,
  // layout_type, …) are a defined contract and must pass through untouched.
  'lessonNotes', 'lesson_notes', 'publishedNotes', 'published_notes',
...
function camelKeys(obj, depth = 0) {
  ...
  for (const [key, val] of Object.entries(obj)) {
    ...
    if ((JSONB_FIELDS.has(key) || JSONB_FIELDS.has(camelKey)) && !Array.isArray(val)) {
      // Pass through JSONB *objects* - user-defined blobs (e.g.
      // content.data) whose internal keys must not be touched. Arrays
      // sharing a JSONB-field name are pagination wrappers
      // (e.g. res.json({ data: [...] }) from list endpoints) and fall
      // through to the array branch below so each row gets transformed.
      result[camelKey] = val;
...
// `totp_enabled` is intentionally NOT here - it's safe and useful to expose.
const SENSITIVE_FIELDS = [
  'password', 'reset_token', 'reset_expires',
  'totp_secret', 'totp_pending_secret', 'totp_backup_codes'
];

// Strip sensitive auth fields from an identity row (user/student/lecturer)
// before returning it. Use at every response site that serialises a raw row
// fetched with select('*'). Returns a shallow copy; leaves the input intact.
function sanitizeIdentity(row) {
...
module.exports = { clean, cleanBulk, camelKeys, sanitizeIdentity };
